			...
			<div class="commentContent w-100">
				...
				<div class="d-flex mt-2">
					<p style="margin:0;margin-right:16px">
                        <a class="" data-bs-toggle="collapse" href="#<?='reply_show_'.$row['id'] ?>" role="button" aria-expanded="false" aria-controls="<?='reply_show_'.$row['id'] ?>"  style="text-transform: uppercase;font-size:14px;font-weight:600;color:rgb(122, 126, 129);" >
                            <i class="fa-solid fa-eye"></i>view replies
                        </a>
                    </p>
					<?php if(isset($_SESSION['auth_user'])) : ?>
					<p style="margin:0">
                        <a class="" data-bs-toggle="collapse" href="#<?='reply_'.$row['id'] ?>" role="button" aria-expanded="false" aria-controls="<?='reply_'.$row['id'] ?>"  style="text-transform: uppercase;font-size:14px;font-weight:600;color:rgb(122, 126, 129);" >
				...
				<div class="mt-1">
					<?php if(isset($_SESSION['auth_user'])) : ?>
					<div class="reply collapse" style="margin:0;padding:0"  id="<?='reply_'.$row['id'] ?>" >
						<div class="d-flex">
							<div>
							<?php
				...
				<input type="hidden" id='replyShowId' value="<?=$row['id'] ?>">
				<div class="replyShow collapse" id="<?='reply_show_'.$row['id'] ?>">
					<?php
					$commentId =$row['id'];
