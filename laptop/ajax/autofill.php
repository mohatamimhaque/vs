...
if(isset($_POST["criteria"])){
    ...
    $search_result = array();
        ...
        foreach($value as $value) {
                $search ="SELECT * from laptop WHERE status='0' AND $value LIKE '%".str_replace(" ", "%", $_POST["criteria"])."%' LIMIT 12";
...
if(isset($_POST["comparesearch"])){
    ...
    $search_result = array();
        foreach($query_run as $value) {
        foreach($value as $value) {
                $search ="SELECT * from laptop WHERE status='0' AND $value LIKE '%".str_replace(" ", "%", $_POST["comparesearch"])."%'";
                $result = mysqli_query($con, $search);
                if(mysqli_num_rows($result) > 0){
                    while($item = mysqli_fetch_array($result)){
                        if (in_array($item['name'], $search_result, TRUE)){
