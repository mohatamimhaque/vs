...
if(isset($_POST["load_data"])){
    $load_data = $_POST["load_data"];
        ...
        <?php
        $query = "SELECT * FROM reply WHERE user_id='$id'";
            ...
            foreach($result as $row){
                ...
                while($table = mysqli_fetch_object($tables)){
                  ...
                  <div style="padding-bottom:6px;border-bottom:1px dotted rgba(0,0,0,0.6);margin-top:12px">
                                      <p style="font-size:13px;color:rgba(0,0,0,0.6);margin:0;padding:0;text-align:start">you reply a comment on <a style="color:rgba(0,0,0,0.9);font-weight:600" href="<?= base_url('trace/show/'.$url) ?>"><?= $name ?></a></p>
                                      ...
                                      <div class="d-flex mt-1">
                                          ...
                                          <p style="font-size:13px;color:rgba(0,0,0,0.7);margin:0;padding:0;text-align:start"><?=$row['reply'] ?></p>
                                          </div>
                                      </div>
                                  </div>
                    <?php
                ...
                ?>
                <div style="padding-bottom:6px;border-bottom:1px dotted rgba(0,0,0,0.6);margin-top:12px">
                    <p style="font-size:13px;color:rgba(0,0,0,0.6);margin:0;padding:0;text-align:start">you reply a comment on <a style="color:rgba(0,0,0,0.9);font-weight:600" href="<?= base_url($table_name.'/show/'.$url) ?>"><?= $name ?></a></p>
                    <small style="font-size:10px;color:rgba(0,0,0,0.7;margin:0;padding:0;line-height:0px);"><?= date('d-M-Y',strtotime($row['created_at'])); ?></small>
                    <div class="d-flex mt-1">
                        <div>
                            <div style='width:45px;height:45px;' class='mt-2 mr-2 d-flex justify-content-center'>
            ...
            for($count = 0; $count < count($page_array); $count++)
            {
              if($page == $page_array[$count])
              {
               ...
               // <script>history.replaceState({},"","?page='.$page_array[$count].'");</script>
                ...
                if($previous_id > 0)
                {
                  $previous_link = '<li><a style="margin-right:30px"  href="javascript:void(0)" data-reply_page_number="'.$previous_id.'">Previous</a></li>';
                }
                ...
                {
                  $next_link = '<li><a  href="javascript:void(0)" data-reply_page_number="'.$next_id.'">Next</a></li>';
                }
              }
              else
              {
                ...
                {
                  $page_link .= '
                  <li><a  href="javascript:void(0)" data-reply_page_number="'.$page_array[$count].'">'.$page_array[$count].'</a></li>
                  ';
                }
              }
            }}
