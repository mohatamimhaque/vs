  ...
  if(isset($_SESSION['mobile_compare'])){
      if(count($_SESSION['mobile_compare'])!=3){
         ...
         if (in_array($_POST["id"], $_SESSION['mobile_compare'], TRUE)){
          ?>
             <p style='margin:0;padding:4px'><i class="fa-solid p-1 rounded-circle fa-xmark mr-1 text-white bg-danger"></i>Warning: you have already added <a href="<?= base_url('mobile/show/'.$row['slug']) ?>"><?=$row['name']?></a> to your product comparision...</p>
          <?php
         }
         else{
            $_SESSION['mobile_compare'][]= $_POST["id"];
            ?>
             <p style='margin:0;padding:4px'><i class="fa-solid p-1 rounded-circle fa-check mr-1 text-white bg-success"></i>success: you have added <a href="<?= base_url('mobile/show/'.$row['slug']) ?>"><?=$row['name']?></a> to your product comparision...</p>
...
if(isset($_POST["comparecart"])){
 ...
 <div class="compareProductList">
    <div class="d-flex justify-content-between compareCartHeader">
      <p>
        Mobile Phones(<?= count($_SESSION['mobile_compare']) ?>)
      </p>
...
<div class="compareCartContent mr-2 d-flex">
          <?php
          if(count($_SESSION['mobile_compare'])<2){
          ?>
          ...
          ?>
          <a href="<?= base_url('mobile/comparison/').$title?>"   class='btn m-auto compareall'>compare</a>
            <?php
          }
          ?>
        </div>
...
if(isset($_POST["comparecart"])){
  ...
  <div class="compareProductList">
    ...
    <div class="compareCartBody">
      <?php
      foreach($list as $l){
        $query = "SELECT image,name FROM mobile WHERE id='$l'";
        $result = mysqli_query($con, $query);
        if(mysqli_num_rows($result) > 0){
             while($row = mysqli_fetch_array($result)){
              $files = $row['image']; 
...
if(isset($_POST["compareSearch"])){
    ...
    $search_result = array();
        ...
        foreach($value as $value) {
                $search ="SELECT * from mobile WHERE status='0' AND $value LIKE '%".str_replace(" ", "%", $_POST["compareSearch"])."%' LIMIT 6";
                    ...
                    while($row = mysqli_fetch_array($result)){
                        if (in_array($row['name'], $search_result, TRUE)){
                            ...
                            <div class="d-flex pp pt-1 justify-content-between">
                              ...
                              <p style="margin:0;color:black;font-size:12px">Mobile</p>
                            </div>
<?php
                        }
                        ?>
