...
session_start();
    ...
    if(isset($_GET['title'])){
      ...
      if(isset($_SESSION[$slug])){
        ...
        $_SESSION[$slug] = $slug;
        mysqli_query($con, "UPDATE mobile SET views=views+1 WHERE slug='$slug'");
      }
      $sql = "SELECT * FROM mobile WHERE slug='$slug'";
              ...
              foreach($query_run as $row){
                        ...
                        <table class="w-100 table table-borderless">
                          ...
                          <tbody>
                              ...
                              <tr>
                                ...
                                <td style=""><?= $row['rear_camera_details'] ?></td>
                              </tr>
                              <tr>
                    ...
                    <div style="" class="productBox d-flex justify-content-between">
                        ...
                        <div class="previewImage">
                          <div>
                          <a class="prodigal-thumbs" href="<?=$url.'admin/upload/category/mobile/image/'.$image[0]?>">
                            <img src="<?=$url.'admin/upload/category/mobile/image/'.$image[0]?>" />
                          </a>
                          </div>
                        </div>
                        ...
                        <?php
                          foreach ($image as $key => $value) {
                            ...
                            ?>
                              ...
                              <a class="prodigal-thumbs" href="<?=$url.'admin/upload/category/mobile/image/'.$image[$key]?>">
                                <img class="thumbnail <?php echo $focused; ?>" src="<?=$url.'admin/upload/category/mobile/image/'.$image[$key]?>" />
                               </a>
                        ...
                        <div class="productDetails p">
                          <div style="overfolow:hidden;position:relative;padding:8px 0;display:flex;justify-content:space-between">
                            ...
                            <button style="" type='submit'  value="<?= $row['id']?>" name="add_to_compare" class="mobile_compare btn"><i class="fa-solid fa-plus"></i> Add to compare</button>
  ...
  <div class="dodo_box" style='width:90%;height:100px;margin:10px auto'></div>
        ...
        <div class="reviews shadowdiv" id="review">
          ...
          <div class="form-group d-flex">
           ...
           <?php if(isset($_SESSION['auth_user'])) : ?>
            <input type="hidden" id="user_id" class="user_id" value="<?= $_SESSION['auth_user']['user_id'];?>" name="user_id">
  ...
  <div class="p_sidebar">
      ...
      <div class="productSuggestBody">
        ...
        <div class="contentbox shadowdiv d-flex" >
          <div class="suggestImage">
            <div>
              <img src="<?= base_url('admin/upload/category/mobile/image/').$image ?>" alt="<?= base_url('admin/upload/category/mobile/image/').$image ?>">
            </div>
          </div>
          <div class="suggestText">
            <div class="">
              <a href="<?= base_url('mobile/show/').$row['slug'] ?>"><p><?= $row['name'] ?></p></a>
              <div class="p">
                <button type='submit' id="" class="mobile_compare btn" value="<?= $row['id'] ?>">
    ...
    <div class="topCompared">
      ...
      <div class="topComparedBody">
        <?php
          $query = "SELECT * FROM comparelist WHERE product_type='mobile' AND product_name LIKE '%".str_replace(" ", "%", $topCompare)."%' ORDER BY views DESC LIMIT 8";
          $query_run = mysqli_query($con, $query);
          if(mysqli_num_rows($query_run) > 0){
            foreach($query_run as $row){
              ?>
...
function load_comment_data(){
    ...
    $.ajax({
        ...
        type:"POST",
        cache:false,
        data:{commentShow:'load_comment',url:url},
        success:function(data){
    ...
    var user_id = $('#user_id').val();
      ...
      else
      {
          $.ajax({
              ...
              method:"POST",
              data:{rating_data:rating_data,user_id:user_id,url:url,heading:heading,summary:summary,category:'mobile'},
              success:function(data){
                  load_rating_data();
                  location.reload();
              }
...
<script>
  ...
  $(document).ready(function(){
    ...
    <?php
      $column='mobile';
      $id = $_SESSION['auth_user']['user_id'];
      $check2 = mysqli_query($con, "SELECT mobile from favourite WHERE user_id='$id'");
